...
function fineBestFittingSubrange(original, replacement) {
  ...
  for (ii = 0; ii <= original.length - replacement.length; ii++) {
    ...
    for (jj = 0; jj < replacement.length; jj++) {
      ...
      console.log(
        `  ${original[ii + jj]} is ${distance} off from ${replacement[jj]}`
      )
...
console.log(fineBestFittingSubrange([2, 5, 9, 1, -3, 40, 2, 19], [10, -3, 39]))

console.log('---Test Run---')
console.log(fineBestFittingSubrange([2, 5, 9, 1, -3, 10, -3, 39], [10, -3, 39]))
